...
fn parse_endpoints_handles_comma_separated_pairs() {
    let v = parse_endpoints("crm@http://localhost:7001/mcp,controlroom@http://localhost:7002/mcp");
    assert_eq!(
        v,
        vec![
...
fn parse_endpoints_skips_malformed_entries() {
    ...
    let v = parse_endpoints("crm@http://x,bad-no-at,@empty-label,empty-url@,controlroom@http://y");
    assert_eq!(
        v,
        vec![
...
fn build_routing_table_errors_on_collision() {
    ...
    let err = build_routing_table(&server_tools).expect_err("should bail on duplicate tool name");
    let msg = format!("{err}");
